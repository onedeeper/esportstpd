---
title: "Individual analysis pro players GSR"
...
setwd("/Users/ymkeverduyn/Desktop/TILBURG UNIVERSITY/YEAR 1 SEM 2/COMPLEX SYSTEMS/Group project/GSR/pros/match_10")
```
...
ggplot(all_players_data, aes(x = Time, y = gsr, color = Player)) +
  ...
  labs(title = "Original gsr Time Series - All Pro players", x = "Time", y = "gsr") +
  theme_minimal()
```
...
ggplot(all_players_data, aes(x = Time, y = Diffgsr, color = Player)) +
  ...
  labs(title = "Discrete Differences of gsr - All Pro Players", x = "Time", y = "Difference in gsr") +
  theme_minimal()
```
...
ggplot(all_density_data, aes(x = x, y = y, color = Player)) +
  geom_line() +
  scale_color_viridis_d() +  # Use discrete viridis color scale
  labs(title = "Density of Discrete Differences in gsr - All Pro Players", x = "Difference in gsr", y = "Density") +
  theme_minimal()
```
...
ggplot(all_players_data, aes(x = Time, y = gsr_vel, color = Player)) +
  geom_line() +
  scale_color_viridis_d() + 
  labs(title = "Change in gsr Time Series - All Pro Players", x = "Time", y = "Velocity of gsr Change") +
  theme_minimal()
```
...
ggplot(all_players_data, aes(x = gsr, y = gsr_vel, color = Player)) +
  ...
  scale_color_viridis_d() +  
  labs(title = "Attractor Dynamics of gsr Change - All Pro Players", x = "gsr", y = "Velocity of gsr Change") +
  theme_minimal()
```
...
ggplot(all_players_data, aes(x = time, y = gsr)) +
  ...
  labs(title = "Vector Density Plot - All Pro Players", x = "Time Window", y = "gsr") +
  theme_minimal() +
