...
# Pro players 
setwd('C:/Users/jette/Documents/Uni/Tilburg/Semester2/Complex Systems/Project CS/HRV_GSR_Final/GSR/pros/match_10')

Pro0 <- read.csv(file="player_0.csv")
Pro1 <- read.csv(file="player_1.csv")
Pro2 <- read.csv(file="player_2.csv")
Pro3 <- read.csv(file="player_3.csv")
Pro4 <- read.csv(file="player_4.csv")
...
mutual(Pro0$gsr, bin, 50, plot = TRUE)
...
embeddim_0 = estimateEmbeddingDim(Pro0$gsr, time.lag = delay_0, max.embedding.dim = 20)
...
mutual(Pro1$gsr, bin, 50, plot = TRUE)
...
```{r}
# Cao, L. (1997). Practical method for determining the minimum embedding dimension of a scalar time series. Physica D: Nonlinear Phenomena, 110(1-2), 43-50.
embeddim_1 = estimateEmbeddingDim(Pro1$gsr, time.lag = delay_1, max.embedding.dim = 20)
...
mutual(Pro2$gsr, bin, 50, plot = TRUE)
...
```{r}
# Cao, L. (1997). Practical method for determining the minimum embedding dimension of a scalar time series. Physica D: Nonlinear Phenomena, 110(1-2), 43-50.
embeddim_2 = estimateEmbeddingDim(Pro2$gsr, time.lag = delay_2, max.embedding.dim = 20)
...
mutual(Pro3$gsr, bin, 50, plot = TRUE)
...
```{r}
# Cao, L. (1997). Practical method for determining the minimum embedding dimension of a scalar time series. Physica D: Nonlinear Phenomena, 110(1-2), 43-50.
embeddim_3 = estimateEmbeddingDim(Pro3$gsr, time.lag = delay_3, max.embedding.dim = 20)
...
mutual(Pro4$gsr, bin, 50, plot = TRUE)
...
```{r}
# Cao, L. (1997). Practical method for determining the minimum embedding dimension of a scalar time series. Physica D: Nonlinear Phenomena, 110(1-2), 43-50.
embeddim_4 = estimateEmbeddingDim(Pro4$gsr, time.lag = delay_4, max.embedding.dim = 20)
...
Players 1 - 4 are pretty similar in terms of their estimated delays and embeddims 
Player 0 falls out of the scheme a little -- choose a value somewhat above the average according to Wallot & Leonardi (2018)
```{r}
...
```
Start with an arbitrary radius of 0.3 -- 0.4 as a compromise between player 0 (still below 1% RR) & 1 (a little above 5% RR)
...
res = drpfromts(Pro1$gsr, Pro3$gsr, windowsize, radius, delay, embeddim, rescale, normalize, mindiagline, minvertline, tw, whiteline, recpt, side = 'both', method = 'crqa', metric = 'euclidean', datatype = 'continuous')

profile = res$profile

plot(-windowsize:windowsize, profile, type = "l", lwd = 5, xlab = "Lags around Line of Synchrony", ylab = "%REC", main = "GSR Pro 1 & Pro 3 Diagonal Recurrence Profile")
...
res = drpfromts(Pro1$gsr, Pro4$gsr, windowsize, radius, delay, embeddim, rescale, normalize, mindiagline, minvertline, tw, whiteline, recpt, side = 'both', method = 'crqa', metric = 'euclidean', datatype = 'continuous')

profile = res$profile

plot(-windowsize:windowsize, profile, type = "l", lwd = 5, xlab = "Lags around Line of Synchrony", ylab = "%REC", main = "GSR Pro 1 & Pro 4 Diagonal Recurrence Profile")
...
res = drpfromts(Pro3$gsr, Pro4$gsr, windowsize, radius, delay, embeddim, rescale, normalize, mindiagline, minvertline, tw, whiteline, recpt, side = 'both', method = 'crqa', metric = 'euclidean', datatype = 'continuous')

profile = res$profile

plot(-windowsize:windowsize, profile, type = "l", lwd = 5, xlab = "Lags around Line of Synchrony", ylab = "%REC", main = "GSR Pro 3 & Pro 4 Diagonal Recurrence Profile")
...
all_players <- as.matrix(cbind(Pro0$gsr, Pro1$gsr, Pro2$gsr, Pro3$gsr, Pro4$gsr))
